...
# ---------------------------------------------------------
# 3. KİMLİK DOĞRULAMA, GİRİŞ VE KAYIT EKRANI
# ---------------------------------------------------------
config_dosya_yolu = '.streamlit/config.yaml'

with open(config_dosya_yolu) as file:
...
if authentication_status:
    ...
    def verileri_getir(kullanici_adi):
        ...
        doc = doc_ref.get()
        if doc.exists:
            return doc.to_dict().get('hisseler', ["FROTO.IS", "THYAO.IS"])
        else:
    ...
    preset_options = {
        "Kendi Listem": kullanici_hisseleri,
    ...
    def hisse_ekle_callback():
        ...
        if input_degeri.strip():
            ...
            guncel_liste = list(set(kullanici_hisseleri + eklenenler))
            verileri_kaydet(username, guncel_liste)
            st.sidebar.success(f"Eklendi ve buluta kaydedildi: {', '.join(eklenenler)}")
    ...
    with st.sidebar.expander("📋 Varlık Seçimi ve Profiller", expanded=True):
        ...
        if st.button("🔄 Kendi Listemi Varsayılana Sıfırla"):
            varsayilan = ["FROTO.IS", "THYAO.IS"]
            verileri_kaydet(username, varsayilan)
            if st.session_state.profil_secim_kutusu == "Kendi Listem":
                st.session_state.secilen_varliklar_multiselect = varsayilan
